...
export function renderNotation(score,options={}){
  ...
  // Reserve label space even when hidden, so toggles never change engraving scale or wrapping.
  const measures=score.measures.map((m,index)=>{const groups=chordGroups(m);return {m,index,groups,...columns(m,groups,{names:true,fingers:true})};}),systems=[];let row=[],used=margin+18;
  ...
  systems.forEach((items,system)=>{
    ...
    const labelCount=data=>Math.max(0,...[...new Set(data.list.map(g=>g.beat))].map(beat=>data.list.filter(g=>!g.rest&&near(g.beat,beat)).reduce((sum,g)=>sum+g.notes.length,0)));
    const extents=hand=>({min:Math.min(...items.map(o=>o[hand].min))-40,max:Math.max(...items.map(o=>o[hand].max+20+15*labelCount(o[hand])))}),right=extents('right'),left=extents('left'),top=26-right.min,bass=top+right.max+32-left.min,height=bass+left.max+38;
...
// One SVG unit has the same physical size in every system, including dense systems.
export function notationLayout(sizes,zoom,minimumWidth=0){
  return {width:Math.max(minimumWidth,...sizes.map(s=>s.width))*zoom,systems:sizes.map(s=>({width:s.width*zoom,height:s.height*zoom}))};
}
// Fit every adjacent pair at that shared scale. Label space is reserved by the renderer.
export function fitTwoSystems(sizes,width,height){
  if(!sizes.length||width<=0||height<=0)return 1;
  const rows=sizes.map(s=>s.height+24),largest=Math.max(...rows.map((h,i)=>h+(rows[i+1]??0)))+60;
