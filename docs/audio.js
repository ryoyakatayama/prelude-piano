...
export class Transport {
 ...
 async unlock(){if(!this.ctx){const C=window.AudioContext||window.webkitAudioContext;if(!C)throw Error('このブラウザは音源再生に対応していません。Safariの最新版でお試しください。');this.ctx=new C();this.master=this.ctx.createGain();this.master.gain.value=.8;const compressor=this.ctx.createDynamicsCompressor();this.master.connect(compressor).connect(this.ctx.destination);}await this.ctx.resume();if(this.ctx.state!=='running')throw Error('音声を開始できませんでした。再生ボタンをもう一度押してください。');}
 async start(score,opts,beat=0){
   ...
   this.initialBeat=beat;this.startTime=this.ctx.currentTime+.045+(opts.countIn?4*60/this.tempo:0);this.anchorTime=this.startTime;this.anchorBeat=beat;this.running=true;this.master.gain.value=clamp(opts.volume??.8,0,1);
   this.resetIndices();
 ...
 stop(){this.generation++;if(this.running)this.lastPosition=this.position();this.running=false;clearInterval(this.timer);cancelAnimationFrame(this.raf);for(const v of this.voices){try{v.stop();}catch{}}this.voices.clear();return this.lastPosition??0;}
}
