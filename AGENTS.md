...
- The user clarified that photos are reference-only and light rendering is a priority. Display one selected photo without playback/loop/fingering/name overlays. Interactive seeking and loop handles belong to the generated notation; keep imported photo coordinates only for compatibility/page lookup.
- Chord notation must share stems for equal onset/duration, separate unequal durations, displace adjacent seconds, and distinguish quarter/eighth notes with flags/beams. Avoid runtime engraving libraries/CDNs.
- All notation systems must use a common pixel scale; never shrink a dense system independently to fit the container. Reserve fingering/name space when hidden so toggling labels preserves dimensions, positions, wrapping and fit-two zoom. Dense fixed-count systems may scroll horizontally instead of shrinking their notes.
